...
export default {
    ...
    run: async (command_parsed: command_parsed_output, msg: Message, client: my_client) => {
        if (command_parsed.form.command) {
            const found_command = client.commands.get(command_parsed.form.command) || client.commands.find((c: command) => c.alias ? c.alias.has(command_parsed.form.command) : false) //check if command is an actual comamnd
            if (!found_command) {
                return simple_embed("can't find", false, "not a real command type help to see all commands")
            }
