...
export default {
  ...
  run: async (
    ...
    await msg.reply(
      "give me the members of team one, as a comma separated list of pings"
    );
    const team_1: string = (
    ...
    const parsed_team_1 = new Set(
      team_1.split(",").map((member) => member.substr(2, member.length - 1))
    );
    await msg.reply(
      "give me the members of team two, as a comma separated list of pings"
    );
    const team_2: string = (
      await msg.channel.awaitMessages(
        (m: Message) => m.author.id === msg.author.id,
    ...
    const parsed_team_2 = new Set(
      team_2.split(",").map((member) => member.substr(2, member.length - 1))
    );
    console.log(parsed_team_2, parsed_team_1);
    ...
    for (let question_number = 0; question_number < 13; question_number++) {
      ...
      const response = await async_collection(
        ...
        (m: Message) =>
          parsed_team_1.has(m.author.id) || parsed_team_2.has(m.author.id)
      );
      console.log(response.message);
      if (response.success) {
        response.message.reply(
          `correct, score is ${
            parsed_team_1.has(response.message.author.id)
              ? round_points.team_a++
              : round_points.team_b++
          }`
        );
        ...
        response.message.reply(
          `no, correct answer was ${question.tossup_answer}`
        );
      }
      await msg.channel.send(
        `team round \n end of question ${question_number} points are ${JSON.stringify(
          round_points
        )}`
      );
    }
