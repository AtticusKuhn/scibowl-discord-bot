...
function parse_commands(msg: string, found_command: command) {
  ...
  const command_args: Array<string> = msg
    .match(/(```(.|\n)*?```|\S+)/g)
    .slice(1); // this regex splits on spaces but does not distrub code blocks
  ...
  if (found_command.form) {
    ...
    const form_captures: Array<string> = found_command.form.match(
      /(\[.*?\]|\<.*?\>|\S+)/g
    );
    ...
    for (let index = 0; index < form_captures.length; index++) {
      ...
      if (argument.indexOf(":") > -1) {
        ...
        switch (datatype) {
          ...
          case "block": {
            if (
              !(
                command_args[word_index].startsWith("```") &&
                command_args[word_index].endsWith("```")
              )
            ) {
              return {
                success: false,
          ...
          case "greedy": {
            ...
            form[name] = command_args
              .slice(index, command_args.length + index - form_captures.length)
              .join(" ");
      ...
      if (!form[name]) form[name] = command_args[index];
      word_index++;
    }
    return {
      success: true,
